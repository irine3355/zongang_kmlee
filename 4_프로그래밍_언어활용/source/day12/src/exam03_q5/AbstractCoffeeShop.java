...
public abstract class AbstractCoffeeShop implements CoffeeShop {
    ...
    public CoffeeShop order() {
        ...
        if (money < price) {
            throw new RuntimeException(menu + "%s를 구입하기에 부족한 금액입니다.");
        }
        ...
        System.out.printf("%s님이 %s에서 %s를 %d원에 주문했습니다. %n", person.getName(), name, price);

        return this;
    }

    public void exit() {
        System.out.printf("%s님이 %s애소 퇴장하였습니다. %n", person.getName(), name);
        person = null;
    }
